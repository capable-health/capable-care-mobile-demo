...
class Goal extends DataWrapper {
  ...
  static mostRecentObservationsByDay(observationTargets) {
    observationTargets
      ...
      .filter((observationTarget) => tomorrow.isAfter(observationTarget.observation.observed_date));
      ...
      return [
        ...new Set(
          this.observations
            .filter(
              (observation) =>
                observation.id == null && tomorrow.isAfter(dayjs.utc(observation.observed_date))
            )

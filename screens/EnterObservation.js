    ...
    <View>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", width: "100%" }}>
        {Object.keys(wellnessScale)
          .sort()
          .map((i) => (
            <WellnessButton key={i} value={value} setValue={setValue} wellnessIndex={i} />
          ))}
      </View>
    </View>
  );
...
const DoneButton = ({ value, setValue, done }) => (
  <TouchableOpacity
    ...
    style={
      value === done ? [styles.observeButton, styles.observeButtonSelected] : styles.observeButton
    }
  >
...
const TargetEntriesWithDate = ({
  ...
  const observationEntries = goal.targets.map((target) => {
    ...
    return (
      <ObservationInputWrapper target={target} value={value} setValue={setValue} key={target.id} />
    );
  });
...
const ActionButtons = ({ goal, observationDate, observationTargets, navigation, datesRefs }) => {
  const save = () => {
    ...
    if (placeholders.length > 0) {
      ...
      if (goal.recurrence) Logger.warn("Saving unbound observation in recurring goal. ", goal);
      ...
      Promise.all(
        goal.targets.map((target) => {
          ...
          return Observation.store({
            observed_value,
            observed_date: datesRefs[observationDate] || dayjs.utc(observationDate, "MM/DD/YYYY"),
            goal_id: goal.id,
            target_id: target.id,

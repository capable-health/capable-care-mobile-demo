...
function Chat({ navigation }) {
  ...
  useEffect(() => {
    ...
    const initConversations = async ({ twilio_auth_token }) => {
      ...
      conversationClient.on("conversationJoined", (joinedConversation) => {
        if (!conversations.find((conversation) => conversation.sid === joinedConversation.sid)) {
      ...
      conversationClient.on("conversationLeft", (leftConversation) => {
        setConversations((existingConversations) =>
          existingConversations.filter((conversation) => conversation.sid !== leftConversation.sid)
        );
      });
    };
    ...
    const fetchToken = () => {
      ...
      const tokenPromise = CapableClient.fetch(GET, `/conversations/twilio_token`).catch((e) =>
        captureError("Failed to get twilio token: ", e)
      );
  ...
  const setFirstConversation = (conversation) => {
    setSortedConversations((sorted_conversations) => [
      conversation,
      ...sorted_conversations.filter((a_conversation) => a_conversation.sid !== conversation.sid),
    ]);
  };

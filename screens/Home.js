...
const HomeHeader = ({ patient }) => {
  return (
    <View style={styles.homeHeader}>
      ...
      />
      <Headline style={styles.greeting}>Welcome{patient.name ? `, ${patient.name}` : ""}!</Headline>
    </View>
  );
};
...
const CarePlanOverview = ({ carePlan, selectedWeek, onSelectWeek, navigation }) => {
  const Item = ({ item, onPress, borderColor, textColor, fontFamily }) => (
    <TouchableOpacity onPress={onPress} style={[styles.tab, { borderColor }]}>
      <Text style={[styles.title, { color: textColor, fontFamily: fontFamily }]}>{item.title}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <Pressable style={[styles.card, styles.toDoItem]} onPress={onTaskPressed}>
      ...
      >
        <Pressable
          style={
            item.isCompleted ? [styles.toDoCheckbox, styles.taskComplete] : styles.toDoCheckbox
          }
          onPress={onTaskPressed}
        >
          <Icon name={item.isCompleted ? "check" : ""} style={{ fontSize: 16, color: "#FFF" }} />
        </Pressable>
      </View>
      <View style={{ flex: 5 }}>
      ...
      <Portal>
        <Dialog visible={visible} onDismiss={hideDialog} style={styles.dialog}>
          <Image
            ...
            resizeMode={"contain"}
          />
          <Dialog.Title style={styles.dialogTitle}>Increase Patient Engagement</Dialog.Title>
